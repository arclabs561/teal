...
describe("cleanArtistName", () => {
  describe("basic cleaning", () => {
    it("should return trimmed input for simple names", () => {
      expect(cleanArtistName("  The Beatles  ")).toBe("Beatles");
    });
  ...
  describe("The prefix removal", () => {
    it("should remove 'The ' prefix", () => {
      expect(cleanArtistName("The Beatles")).toBe("Beatles");
      expect(cleanArtistName("The Rolling Stones")).toBe("Rolling Stones");
    });

    it("should be case insensitive for 'The '", () => {
      expect(cleanArtistName("THE BEATLES")).toBe("BEATLES");
      expect(cleanArtistName("the beatles")).toBe("beatles");
    });

    it("should NOT remove 'The' if it's the entire name", () => {
      expect(cleanArtistName("The")).toBe("The");
    });

    it("should handle 'The The' band name", () => {
      // Edge case: "The The" is a real band
      expect(cleanArtistName("The The")).toBe("The");
    });
  });
...
describe("normalizeForComparison", () => {
  ...
  describe("special character handling", () => {
    ...
    it("should handle Japanese characters", () => {
      // Japanese characters are filtered out, leaving empty or partial
      const result = normalizeForComparison("きゃりーぱみゅぱみゅ");
      expect(result).toBe("");
    });

    it("should preserve Latin characters mixed with special chars", () => {
      expect(normalizeForComparison("CHVRCHΞS")).toBe("chvrchs");
    });
  });
...
describe("unhandled/documented nuances", () => {
  ...
  describe("punctuation in names", () => {
    it("should preserve exclamation marks in names", () => {
      ...
      // Note: "The !!!" becomes "!!!" after "The " removal
      expect(cleanArtistName("The !!!")).toBe("!!!");
    });
  ...
  describe("CJK (Chinese/Japanese/Korean) handling", () => {
    ...
    it("normalizeForComparison filters out non-Latin characters", () => {
      // NUANCE: CJK characters are filtered out during normalization
      // This may cause matching issues for non-Latin music
      expect(normalizeForComparison("花火")).toBe("");
...
describe("threshold boundary tests (overfitting check)", () => {
  ...
  describe("diverse length inputs", () => {
    ...
    it("should handle long track with guff", () => {
      ...
      expect(cleanTrackName(longWithGuff)).toBe("This Is An Extremely Long Track Name That Goes On And On");
    });
  });
});
...
describe("diverse real-world inputs (overfitting check)", () => {
  ...
  describe("international artists", () => {
    ...
    it("should handle Nordic names", () => {
      ...
      expect(cleanArtistName("The Sigur Rós")).toBe("Sigur Rós");
    });
  });
...
describe("real test cases from evaluation data", () => {
  ...
  describe("remix tracks", () => {
    ...
    it("should handle 'Sensation - Rrose Remix' by Daniel Avery", () => {
      // Dash-separated remix format (not parenthesized)
      expect(cleanTrackName("Sensation - Rrose Remix")).toBe("Sensation - Rrose Remix");
    });

    it("should handle 'All Of My - Aries Remix' by Tom & Jerry", () => {
  ...
  describe("parenthetical tracks", () => {
    ...
    it("should handle 'Let The Sunshine In (Reprise) - Remastered 2000' by The 5th Dimension", () => {
      // "(Reprise)" is guff, "- Remastered 2000" is also guff
      // But this is complex - let's see what actually happens
      const result = cleanTrackName("Let The Sunshine In (Reprise) - Remastered 2000");
      // "Reprise" is in guff list, so it should be removed
...
describe("integration scenarios", () => {
  ...
  describe("matching after cleaning", () => {
    it("should enable matching of cleaned vs original", () => {
      const original = "Bohemian Rhapsody (2011 Remaster)";
      const cleaned = cleanTrackName(original);
      const mbResult = "Bohemian Rhapsody";
      
      expect(normalizeForComparison(cleaned)).toBe(normalizeForComparison(mbResult));
    });
